...
class BootBot extends EventEmitter {
   ...
   * @returns {Promise}
   */
  getUserProfile(userId, scope) {
    if(scope == undefined || scope == null){
      scope = [ 'name', 'first_name', 'last_name', 'profile_pic' ];
    }
    scope = scope.join(',');
    
    const url = `https://graph.facebook.com/${this.graphApiVersion}/${userId}?fields=${scope}&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
